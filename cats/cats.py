...
def accuracy(typed, source):
    ...
    for i in range(len(source_words)):       
        if i < len(typed_words) and typed_words[i] == source_words[i]:
            count += 1
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        for i in range(len(word_list)):
            dis = diff_function(typed_word,word_list[i],limit)
            if dis < minimum:
                minimum = dis
                loc = i
        if minimum > limit:
...
def furry_fixes(typed, source, limit):
    ...
    if len(typed) != len(source):
        l = min(len(typed), len(source))
        return furry_fixes(typed[0:l],source[0:l], limit-abs(len(typed) - len(source))) + abs(len(typed) - len(source))
    elif len(typed) == 0:
        return 0
    elif limit < 0:
        return 1
    elif typed[0] == source[0] and len(typed) == 1:
        return 0
    elif typed[0] != source[0] and len(typed) == 1:
        return 1
    elif typed[0] == source[0] and len(typed) != 1:
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = [[tpp[i][j] - tpp[i][j-1] for j in range(1, len(tpp[i]))] for i in range(len(tpp))]  # You may remove this line
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    re={i:[] for i in player_indices}
    
    def find(word_index):
        minimum = 9999999
        for i in player_indices:
            if get_time(times, i, word_index) < minimum:
                minimum = get_time(times, i, word_index)
                loc = i
        return loc
    
    for i in word_indices:
        re[find(i)].append(words[i])
    lis=[]
    for i in re:
        lis.append(re[i])
    return lis
